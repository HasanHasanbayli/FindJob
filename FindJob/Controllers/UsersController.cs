...
public class UsersController : Controller
{
    ...
    public async Task<IActionResult> Index()
    {
        ...
        AppUser user = await _userManager.FindByNameAsync(User.Identity.Name);
        var postJobs = _db.AppUserPostJobs.Where(x => x.AppUserId == user.Id).ToList();
        var users = _userManager.Users.ToList();
        var StaredUsers = new List<AppUser>();
        var AppliedUsers = new List<AppUser>();

        foreach (AppUserPostJob j in postJobs)
        foreach (AppUser u in users)
            if (u.Id == j.AppendUserId && j.IsFavorite)
                StaredUsers.Add(u);
        foreach (AppUserPostJob j in postJobs)
        foreach (AppUser u in users)
            if (u.Id == j.AppendUserId && j.IsContacted)
                AppliedUsers.Add(u);
        PostJobVM postJobVM = new()
        {
            AppUserPostJobs = _db.AppUserPostJobs.Include(x => x.AppUser).Where(x => x.PostJob.AppUserId == user.Id)
                .ToList(),
    ...
    public async Task<IActionResult> UpdateProfile(UpdateVM update)
    {
        ...
        if (User.Identity.IsAuthenticated)
        {
            ...
            if (update.Photo != null)
            {
                ...
                if (user.Image != null) Helper.DeleteImage(_env.WebRootPath, path, user.Image);
            ...
            {
                if (ModelState["Resume"].ValidationState == ModelValidationState.Invalid) return View();
                ...
                if (user.Resume != null) Helper.DeleteImage(_env.WebRootPath, path, user.UserResume);
    ...
    public async Task<IActionResult> MyJobList(int? page)
    {
        ...
        return View(_db.PostJobs.Include(p => p.AppUserPostJobs).OrderByDescending(p => p.Id).Skip(((int) page - 1) * 5)
            .Take(5).ToList());
    ...
    public IActionResult FindStaff(AppUser user, int? page)
    {
        ...
        ViewBag.Page = page;
        if (page == null)
            return View(_userManager.Users.Where(x => x.IsCompany == false).OrderByDescending(p => p.Id).Take(6)
                .ToList());
        return View(_userManager.Users.Where(x => x.IsCompany == false).OrderByDescending(p => p.Id)
    ...
    {
        AppUser user = _userManager.Users.Include(p => p.PostJobs).Where(x => x.IsCompany == false)
    ...
    {
        AppUser user = _userManager.Users.Where(x => x.IsCompany == true).FirstOrDefault(z => z.Id == id);
        return View(user);
    }
